...
export class GeneradasLiquidadoresComponent {
  ...
  loadDataCM(): void {
    ...
    if (fecha_desde && fecha_hasta) {
      ...
      this.http
        .post(API_ROUTES.RENTAS.REPORTE_EXONERACIONES, body.toString(), {
          headers,
        })
        .subscribe(
          (response: any) => {
          ...
          (error) => {
            ...
            Swal.fire({
              ...
              text:
                error.error?.msg || "Ocurrió un error al realizar la consulta.",
            });
          }
        );
    } else {
  ...
  exportarExcel() {
    const fecha_desde = this.formularioMarcaciones.get("fecha_desde")?.value;
    const fecha_hasta = this.formularioMarcaciones.get("fecha_hasta")?.value;
    const cedula = this.formularioMarcaciones.get("usuario")?.value;
    ...
    const data = this.marcaciones.map((m) => ({
      FECHA_EXONERACION: m.FECHA_EXONERACION?.split(" ")[0],
    ...
    const workbook: XLSX.WorkBook = {
      Sheets: { Exoneraciones: worksheet },
      SheetNames: ["Exoneraciones"],
    };
  ...
  generarPDF() {
    ...
    const datosExoneraciones = this.marcaciones.map((item, index) => ({
      ...
      "Fecha Exoneración": item.FECHA_EXONERACION
        ? new Date(item.FECHA_EXONERACION).toLocaleDateString()
        : "-",
    }));
    ...
    this.reporteExoneracionesService
